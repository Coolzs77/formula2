...
**更新时间**: 2025-07-16 16:17:19 北京时间  
**用户**: Coolzs77  
**版本**: v2.1

## 1. 项目概述

本项目是一个基于深度学习的手写数学公式识别系统，能够识别手写的数字(0-9)和数学符号(+、-、×、÷、.)，并进行公式识别和计算。系统使用残差卷积神经网络(ResNet-based CNN)，支持从文件加载图像和画布手写输入两种识别方式。
...
### 5.1 数据集生成
由于generator.py已删除，您需要准备自己的数学符号数据集，或使用现有的符号数据。
...
- **左侧下方**: 手写输入区域，支持鼠标绘制
- **右侧上方**: 操作按钮(加载图像、识别公式、清除画布)
- **右侧中间**: 识别结果显示(公式、LaTeX、计算结果)
- **右侧下方**: 历史记录区域(可折叠)
